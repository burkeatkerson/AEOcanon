...
/**
 * The AEO Scorecard interpretation framework — the system prompt the model
 * interprets each lead's ratings through. This is the product: the model does
 * not invent advice, it applies this fixed framework in the Canon voice.
 *
 * Embedded as a versioned constant (rather than read from disk at runtime) so
 * the prompt is bundled deterministically with the code and can't go missing in
 * a serverless build. The authored source of truth lives beside this file at
 * `interpretation-framework.md` — keep the two in sync when editing.
 */
export const INTERPRETATION_FRAMEWORK = `# AEO Scorecard — Interpretation Framework
...
/**
 * Appended to the user message on the no-website branch (so the cached framework
 * system block stays byte-identical across branches). Explains how to apply the
 * eight-pillar lens to a business that has no site to rate yet.
 */
export const NO_WEBSITE_ADDENDUM = `IMPORTANT — NO-WEBSITE CASE. This business does not have a website yet, so there are NO eight-pillar ratings and no overall score/tier. Do not invent ratings or a percentage. Interpret their off-site answers through the same lens and the same §5 stakes calibration.
